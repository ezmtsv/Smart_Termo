...
////////////////////////////////////////
/*			
			if(Report_buf[4]!=0x74 && Report_buf[4]!=0x75 && Report_buf[4]!=0x76){				///команда 0x74, 0x75 для работы с ком портом
...
*/
			if(Report_buf[4]==0x74){								// команда на запись в ком порт
			...
			if(Report_buf[4]==0x75)	{							// команда на чтение из буфера ком
				flag_read_buf_COM = true;
			}
			if(Report_buf[4]==0x76)	{							// команда на передачу данных клиенту
				len_packet_COM = Report_buf[6];
				for(k = 0; k<len_packet_COM; k++){
					data_from_soft[k]= Report_buf[k+7];
				}
			...
			if(Report_buf[4]==0x99)	{							// передать данные о дате последней загруженной прошивке
...
///////////////////////////////////
  ...
  led_one_OFF(LED_R);
//	if(!flag_paket_for_COM && !flag_read_buf_COM && !flag_start_sendWIFI)TIM_Cmd(TIM3, ENABLE);	 // включаем таймер после отработки команды для ком порта
...
/// A0E<<4 = 0xD0, -A1E = 0x8C, -A2E = 0x87, -A3E = 0x84, A4E = 0 
 uint8_t dim_koef_def[20]= {0xd5, 0xc3, 0x6b, 0x75, 0x8b, 0x69, 0x5f, 0x36, 0x87, 0x19, 0x7c, 0x84, 0x84, 0x81, 0xe8, 0x10, 0, 0x7c, 0xbf, 0};    
	...
	j = 0;
	for(i = 0; i<5; i++){																// модифицируем нужный нам пакет
